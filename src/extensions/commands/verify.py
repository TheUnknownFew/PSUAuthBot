...
class Verify(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        self.__loaded_status_views: dict[int, UserStatusView] = {}
        self.check_for_email_replies.start()

    def __on_view_termination(self, user: User):
        self.__loaded_status_views.pop(user.id)

    def __make_status_view(self, user_entry: UserEntry) -> UserStatusView:
        self.__loaded_status_views[user_entry.user_id] = UserStatusView(user_entry, self.__on_view_termination)
        self.__bot.add_view(self.__loaded_status_views[user_entry.user_id], message_id=user_entry.status_msg_id)
        return self.__loaded_status_views[user_entry.user_id]
        ...
        if not self.__are_status_views_loaded:
            async with UserEntryManager() as users:
                async for user_entry in users.get_unverified_users():
                    self.__make_status_view(user_entry)
    ...
    async def listen_for_canvas_embed(self, message: Message) -> None:
        ...
        if self.__bot.user.id != message.author.id:
            ...
            if message.guild is None and has_status:
                ...
                await self.__loaded_status_views[user_entry.user_id].update_status_message(new_data=user_entry)
    ...
    async def check_for_email_replies(self):
        ...
        async for user_entry in unverified_users:
            user = await user_entry.user
            if await userdb.is_user_registered_with_status(user, userstatus.is_user_pending_email):
                async with UserEntryManager(user) as user_manager:
                    self.__gmail.check_for_replies(user_entry)(user_entry, user_manager, self.__loaded_status_views[user_entry.user_id])
    ...
    async def verify(self, ctx: commands.Context, first_name: str, last_name: str, email: str, confirm_email: str):
        ...
        async with UserEntryManager(ctx.author) as user:
            if not user.is_registered:
                ...
                await status_message.edit(embed=await emb.create_status_message(user_entry), view=self.__make_status_view(user_entry))
